...
Ese script levanta la BD local, reconstruye `backend` y `frontend` sin cache, deja Prometheus y Grafana arriba y aplica los seeds locales versionados.
...
```powershell
.\Producto\tests\load\scripts\run-k6-smoke.ps1
```
...
Arbol consolidado de archivos nuevos relevantes para esta implementacion, confirmado sobre el filesystem actual:
...
Los JSON de `tests/load/k6/results/` se generan localmente en cada corrida y ya no se versionan en Git.
