...
def test_verify_and_decode_jwt_success(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    ...
    token = _token(secret="test-secret-key-with-at-least-32-bytes", audience="bot-panol")
...
def test_verify_and_decode_jwt_invalid_signature(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    _configure(monkeypatch)
    token = _token(secret="wrong-secret-key-with-at-least-32-bytes", audience="bot-panol")
...
def test_verify_and_decode_jwt_invalid_issuer(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    _configure(monkeypatch)
    token = _token(secret="test-secret-key-with-at-least-32-bytes", issuer="otro-issuer", audience="bot-panol")
    with pytest.raises(TokenValidationError):
        verify_and_decode_jwt(token)
...
def test_verify_and_decode_jwt_expired(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    _configure(monkeypatch)
    token = _token(secret="test-secret-key-with-at-least-32-bytes", audience="bot-panol", exp_delta=-60)
    with pytest.raises(TokenValidationError):
        verify_and_decode_jwt(token)
...
def test_verify_and_decode_jwt_nbf_in_future(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    _configure(monkeypatch)
    token = _token(secret="test-secret-key-with-at-least-32-bytes", audience="bot-panol", nbf_delta=120)
    with pytest.raises(TokenValidationError):
        verify_and_decode_jwt(token)
...
def test_verify_and_decode_jwt_requires_bot_token_use(monkeypatch) -> None:  # type: ignore[no-untyped-def]
    _configure(monkeypatch)
    token = _token(
        secret="test-secret-key-with-at-least-32-bytes",
        audience="bot-panol",
        token_use="web-access",
    )
    with pytest.raises(TokenValidationError):
        verify_and_decode_jwt(token)
